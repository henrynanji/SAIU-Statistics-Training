...
## Task 1
frequency_table_count <- frequency_table_count %>% ## taking the frequency table we made earlier and assigning it to the same variable name. This means we will overwrite the original table. If you wanted to keep the original table with the original columns names as well, you can assign a different variable name (i.e. change the first name closest to the left)
  rename( ## using the rename function
    Diagnosis = Diagnosis_ICD10_Description,  ## rename the column to something easier to understand. If you are using multiple words, join with _ or encase in ""
...
frequency_table_count <- frequency_table_count %>% ## taking the frequency table we made earlier and assigning it to the same variable name. This means we will overwrite the original table. If you wanted to keep the original table with the original columns names as well, you can assign a different variable name (i.e. change the first name closest to the left)
  rename( ## using the rename function
    "Diagnosis Description" = Diagnosis_ICD10_Description,  ## rename the column to something easier to understand. If you are using multiple words, join with _ or encase in ""
    "Count of Inpatients" = Count
  )
...
frequency_table_count <- frequency_table_count %>% ## taking the frequency table we made earlier and assigning it to the same variable name. This means we will overwrite the original table. If you wanted to keep the original table with the original columns names as well, you can assign a different variable name (i.e. change the first name closest to the left)
  rename( ## using the rename function
    Diagnosis_Description = Diagnosis_ICD10_Description,  ## rename the column to something easier to understand. If you are using multiple words, join with _ or encase in ""
    'Count of Inpatients' = Count
  )
...
frequency_table_count_Daignosis_Admittance <- discrete_data %>%
  group_by(Diagnosis_ICD10_Description, Ward_Transferred_To, Sex) %>% 
  summarise(Count = n()) %>%
  rename(
    'Diagnosis Description' = Diagnosis_ICD10_Description, 
    'Ward Admitted to (incl. SDEC)' = Ward_Transferred_To,
    'Count of Inpatients' = Count
  )
...
## Question 4:
## We don't want a scale on the inpatient axis, we want just 0 and 1. To acheive this, we need to convert
...
## Task 6:
ggplot(discrete_data, aes(x = Admitted_To_Ward, fill = Ethnicity)) + ## we have added fill = Sex to split the stacked bars by sex
  geom_bar(color = "black") + ## stacked bars (rather than clustered)
  labs(
    title = "Count of Inpatients by Ward Admittance Status 
(1 = admitted to ward, 0 = SDEC admission) and Ethnicity", 
    x = "Admitted to Ward", 
    y = "Count of Patients"
  ) +
  theme_minimal() + 
...
ggplot(frequency_table_prop, aes(x = "", y = Count, fill = Ethnicity)) +
  geom_bar(stat = "identity", width = 1) +
  coord_polar(theta = "y") +
  labs(title = "Pie Chart Showing % Proportion of Inpatients by Ethnicity", fill = "Ethnicity") +
  theme_void() +
  theme(
  ...
  geom_text(aes(label = Label), position = position_stack(vjust = 0.5), size = 3) # Adjust text size as needed
