...
discrete_data <- read_excel('Data for Statistics Training NNICB - Dummy Discrete and Non-Identifiable Ambulance Continuous33.xlsx', sheet = "Synthetic_Discrete_Inpatient")

continuous_data <- read_excel('Data for Statistics Training NNICB - Dummy Discrete and Non-Identifiable Ambulance Continuous33.xlsx', sheet = "Continuous_Ambulance_Handovers")
...
new_table_name <- discrete_data %>% ## assign a new table name here e.g. frequency_table_count_ethnicity
  group_by(Diagnosis_ICD10_Description, new_grouping_variable) %>% ## change grouping variable. This should be a column from discrete_data
  summarise(Count = n()) %>%
  rename(
    'Diagnosis Description' = Diagnosis_ICD10_Description, 
    'Count of Inpatients' = Count
  )
View(new_table_name) ## use correct table name HINT: use the table name you assigned in line 83




## TASK 3: Create a new frequency table that counts patients per diagnosis, but grouped by 
## at least 2 variables (try using a new one) and rename any columns that aren't easy to understand. 
## To look at the column names again, access the data via Environment. 

## HINT: Assign a table name on line 104 and add new variables to line 105 after the diagnosis description variable. 
## To rename the new columns we added a comma to the end of line 109 and renamed the other columns beneath this. If you don't want to rename the new column(s) you can comment out lines using #. Remember to make sure the last rename line does not have a comma at the end
## Don't forget to call your table name in the brackets in line 113. 
...
new_table_name <- discrete_data %>% ## add table name
  group_by(Diagnosis_ICD10_Description, new_grouping_variable1, new_grouping_variable2) %>% 
  summarise(Count = n()) %>%
  rename(
    'Diagnosis Description' = Diagnosis_ICD10_Description,
    'New name 1' = new_grouping_variable1, ## optionally you can rename the new variable column names. We didn't do this previously as Sex and Ethnicity were already appropriatley named
    'New name 1' = new_grouping_variable2,
    'Count of Inpatients' = Count
  )
View(new_table_name) ## calling table
...
ggplot(discrete_data, aes(x = Admitted_To_Ward)) + ## change the x value here to a new variable
  geom_bar(fill = "skyblue", color = "black") + ## add colour to the bars. change these colours if you wish. Google R colours
  labs(title = "Count of Inpatients by Ward Admittance Status 
(1 = admitted to ward, 0 = SDEC admission)", x = "Inpatient", y = "Count of Patients") + ## chang the title and axis labels accordingly
  theme_minimal() + 
  theme(
    plot.title = element_text(size = 30),      ## title text size
    axis.title.x = element_text(size = 30, margin = margin(t = 20)),    ## X-axis title text size
...
## Admittance status split by sex
...
## TASK 6: Split the stacked bar chart by the ward that each inpatient was transferred to (Ward_Transferred_To) instead of sex
## HINT: don't forget to change the title

ggplot(discrete_data, aes(x = Admitted_To_Ward, fill = Sex)) + ## change Sex to Ward_Transferred_To
  geom_bar(color = "black", position = "stack") + ## stacked bars (rather than clustered)
  labs(
    title = "Count of Inpatients by Ward Admittance Status 
(1 = admitted to ward, 0 = SDEC admission) and Sex", ## change title name 
    x = "Admitted to Ward", 
    y = "Count of Patients"
  ) +
  theme_minimal() + 
...
## this is the same proportion frequency table as before, but we have added a new column on line 278
frequency_table_prop <- discrete_data %>%
  group_by(Admitted_To_Ward) %>%
  summarise(Count = n()) %>%
...
ggplot(frequency_table_prop, aes(x = "", y = `Proportion (%)`, fill = Admitted_To_Ward)) + ## we have stated the data source (frequency_table_prop) and columns within this that we want to use. X axis is empty because it's a pie chart so there isn't really an x axis. y will be our segments and fill is like the legend.
  ...
  labs(fill = "Ward Admittance Status", title = "Pie Chart Showing % Proportion of Inpatients by Ward Admittance Status
       1 = Admitted to ward, 0 = SDEC admission") ## chart labels
...
ggplot(frequency_table_prop, aes(x = "", y = `Proportion (%)`, fill = Admitted_To_Ward)) + ## we have stated the data source (frequency_table_prop) and columns within this that we want to use. X axis is empty because it's a pie chart so there isn't really an x axis. y will be our segments and fill is like the legend.
  ...
  labs(fill = "Ward Admittance Status", title = "Pie Chart Showing % Proportion of Inpatients by Ward Admittance Status
       1 = Admitted to ward, 0 = SDEC admission") + ## chart and legend title 
  theme_void() +
  theme(
  ...
  geom_text(aes(label = Label), position = position_stack(vjust = 0.5), size = 8, color = "white") ## add data labels and adjust text size and colour. We are using our previously created Label column



## TASK 7: Complete the ED_Location proportion table (outline provided below) to make a pie chart showing the 
## proportion of Inpatients by ED_Location 
## HINT: Change the fill variable and the title. Ask for help/look at model answers if you get stuck

frequency_table_prop <- discrete_data %>%
  group_by(variable_name) %>% ## replace with the correct column name from discrete_data
  summarise(Count = n()) %>%
  mutate(`Proportion (%)` = (Count / sum(Count) * 100),
         Label = paste0(round(`Proportion (%)`, 1), "%"))

ggplot(frequency_table_prop, aes(x = "", y = `Proportion (%)`, fill = )) + ## data source and columns. Add in the column we want to use for fill
  geom_bar(stat = "identity", width = 1) + ## format the pie chart segments
  coord_polar(theta = "y") + ## make the chart a pie chart - adds the circular element 
  labs(title = "Pie Chart Showing % Proportion of ", fill = "") + ## chart and legend title. Complete the title 
  theme_void() +
  theme(
    legend.title = element_blank(), ## no legend title
    legend.text = element_text(size = 30), ## legend text size
  ...
  geom_text(aes(label = Label), position = position_stack(vjust = 0.5), size = 8, color = "white") ## data labels
...
q <- hist(continuous_data$Handover_Minutes, ## specify the data source. Assign histogram to random variable q (for customisation below)
          breaks = n, ## the method to create your bins. Replace n with the number fo bins you want
          xlab = 'Handover Time (mins)', ## x axis title
          ylab = 'Frequency', ## y axis title
          main = 'Distribution of EMAS Ambulance Handover Times in 2023/24 FY', ## chart title
          col = "skyblue",       ## colour of bars
...
## create box plot of EMAS handover times across each day 
...
sd(continuous_data$Handover_Minutes)
